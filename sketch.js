...
function renderKeyboardImages() {
  ...
  for (let i = 0; i < 26; i++) {
    ...
    let ki = createGraphics(100, 100);
    ki.background(215);
    ki.noStroke();
    for (let y = 0; y < 10; y++) {
      for (let x = 0; x < 10; x++) {
        let box = y * 10 + x;
        let guessed = getGuessed(box);
        if (guessed[2].includes(letter)) {
          ki.fill(10, 180, 10);
          ki.rect(x * 10, y * 10, 10, 10);
        } else if (guessed[1].includes(letter)) {
          ki.fill(180, 180, 10);
          ki.rect(x * 10, y * 10, 10, 10);
        } else if (guessed[0].includes(letter)) {
          ki.fill(120);
          ki.rect(x * 10, y * 10, 10, 10);
        }
      }
    }
    keyboardImages[letter] = ki;
  }
}

function getGuessed(box) {
  let guessed = [[], [], []]
  for (let i = 0; i < game.guesses.length; i++) {
    let status = getWordStatus(game.words[box], game.guesses[i]);
    for (let j = 0; j < 5; j++) {
      if (!guessed[status[j]].includes(game.guesses[i][j])) {
        guessed[status[j]].push(game.guesses[i][j]);
      }
    }
  }
  return guessed;
...
function mousePressed() {
  if (getCompletedWords() == 100) {
    if (mouseX > width / 2 - 180 && mouseY > height / 2 - 40 && mouseX < width / 2 - 180 + 360 && mouseY < height / 2 - 40 + 80) {
      ...
      for (let y = 0; y < 10; y++) {
        for (let x = 0; x < 10; x++) {
          copyText += (game.guesses.indexOf(game.words[y * 10 + x]) + 1).toString().padEnd(3) + ' ';
        }
        copyText += '\n';
      }
      copyText += '\nhttps://hked.live/centordle';
...
function keyPressed() {
  ...
  if (keyCode == BACKSPACE || keyCode == DELETE) {
    if (game.currentGuess.length > 0) {
      game.currentGuess = game.currentGuess.substring(0, game.currentGuess.length - 1);
    }
  }
  if (keyCode == ENTER || keyCode == RETURN) {
    if (game.currentGuess.length == 5 && isWordValid() == 1) {
...
function keyTyped() {
  if (getCompletedWords() == 100 || game.guesses.length >= 108) {
    return;
  }
  if ((/[a-zA-Z]/).test(key)) {
...
function isWordValid() {
  return game.currentGuess.length == 5 ? (words.includes(game.currentGuess) ? 1 : 2) : 0;
}

function getCompletedWords() {
  let count = 0;
  for (let i = 0; i < game.words.length; i++) {
    if (game.guesses.includes(game.words[i])) {
      count += 1;
    }
  }
  return count;
...
function beat() {
  let oldRenderImage = renderKeyboardImages;
  renderKeyboardImages = function() {};
  for (let i = 0; i < game.words.length; i++) {
    console.log(i);
    game.currentGuess = game.words[i];
    keyCode = ENTER;
    keyPressed();
  }
  renderKeyboardImages = oldRenderImage;
  renderKeyboardImages();
}

function getWordStatus(word, guess) {
  if (word == undefined || guess == undefined) {
    return [0, 0, 0, 0, 0];
  }
  let wordCounts = {};
  for (let i = 0; i < 5; i++) {
    if (wordCounts[word[i]] == undefined) {
      wordCounts[word[i]] = 1;
    } else {
      wordCounts[word[i]] += 1;
    }
  ...
  for (let i = 0; i < 5; i++) {
    if (word[i] == guess[i]) {
      status[i] = 2;
      if (wordCounts[guess[i]] !== undefined) {
        wordCounts[guess[i]] -= 1;
      }
    }
  }
  for(let i = 0; i < 5; i++) {
...
function draw() {
  ...
  if (width < height) {
    ...
    text("This game is not intended for mobile devices", width / 2, height / 2 - 20);
    text("Come back when you have access to a computer", width / 2, height / 2 + 20);

    fill(0);
    textAlign(CENTER, CENTER);
    textStyle(BOLD);
    textSize(32);
    text("CENTORDLE ", width / 2, 25);
    return;
  }
  ...
  let boxWidth = (width - width / 4) / 10;
  let boxHeight = (height - 200) / 10;
  let innerBoxWidth = boxWidth / 5;
  let innerBoxHeight = boxHeight / 2;

  let selectBoxX = floor((mouseX - width / 4) / ((width - width / 4) / 10));
  let selectBoxY = floor((mouseY - 50) / ((height - 200) / 10));

  for(let y = 0; y < 10; y++) {
    for(let x = 0; x < 10; x++) {
      let i = y * 10 + x;
      let boxWord = game.words[i];
      let lastGuess = game.guesses[game.guesses.length - 1];
      let wordStatus = getWordStatus(boxWord, lastGuess);

      let topX = x * boxWidth + width / 4;
      let topY = y * boxHeight + 50;

      fill(230);
      noStroke();
      rect(topX + 2, topY + 2, boxWidth - 4, boxHeight - 4, 5);
      
      fill(160);
      for (let iy = 0; iy < 2; iy++) {
        for (let ix = 0; ix < 5; ix++) {
          let boxTopX = topX + ix * innerBoxWidth;
          let boxTopY = topY + iy * innerBoxHeight;
          
          if (iy == 0) {
            if (!game.guesses.includes(boxWord)) {
              ...
              rect(boxTopX + 2, boxTopY + 2, innerBoxWidth - 4, innerBoxHeight - 4, 1);

              if (game.guesses.length > 0) {
                fill(0);
                textSize(min(innerBoxHeight / 1.5, innerBoxWidth / 1.5));
                text(lastGuess[ix].toUpperCase(), boxTopX + innerBoxWidth / 2, boxTopY + innerBoxHeight / 2);
              }
            } else {
              fill(10, 180, 10);
              rect(boxTopX + 2, boxTopY + 2, innerBoxWidth - 4, innerBoxHeight - 4, 1);
              ...
              fill(0);
              textSize(min(innerBoxHeight / 1.5, innerBoxWidth / 1.5));
              text(boxWord[ix].toUpperCase(), boxTopX + innerBoxWidth / 2, boxTopY + innerBoxHeight / 2);
            }
          } else {
            if (!game.guesses.includes(boxWord)) {
              fill(224);
              ...
              if (ix < game.currentGuess.length) {
                text(game.currentGuess[ix].toUpperCase(), boxTopX + innerBoxWidth / 2, boxTopY + innerBoxHeight / 2);
              }
            } else {
              fill(224);
              rect(boxTopX + 2, boxTopY + 2, innerBoxWidth - 4, innerBoxHeight - 4, 1);
              ...
              fill(0);
              textSize(min(innerBoxHeight / 1.5, innerBoxWidth / 1.5));
              let guesses = (game.guesses.indexOf(boxWord) + 1).toString();

              if (ix < guesses.length) {
                text(guesses[ix], boxTopX + innerBoxWidth / 2, boxTopY + innerBoxHeight / 2);
              }
            }
          }
      ...
      if (game.guesses.length > 0) {
        let guessed = getGuessed(i);
        let usedLetters = [];
        for (letter of guessed[2]) {
          usedLetters.push(letter);
          fill(10, 180, 10);
          rect(topX + boxWidth - 2 - 10 * usedLetters.length, topY + boxHeight - 12, 8, 8, 3);
        }
        for (letter of guessed[1]) {
  ...
  if (selectBoxX >= 0 && selectBoxY >= 0 && selectBoxX < 10 && selectBoxY < 10) {
    let i = selectBoxY * 10 + selectBoxX;
    let bw = width / 8 / 5;
    let bh = (height - 60) / 54;
    for (let y = 0; y < 108; y++) {
      if (y >= game.guesses.length) {
        fill(230);
        for (let x = 0; x < 5; x++) {
          rect(x * bw + 1 + floor(y / 54) * width / 8, (y % 54) * bh + 52, bw - 2, bh - 2, 1);
        } 
      } else {
        let boxWord = game.words[i];
        let guess = game.guesses[y];
        let wordStatus = getWordStatus(boxWord, guess);
        for (let x = 0; x < 5; x++) {
          if (wordStatus[x] == 2) {
            fill(10, 180, 10);
          } else if (wordStatus[x] == 1) {
            fill(180, 180, 10);
          } else {
            fill(120);
          }
          ...
          fill(0);
          textSize(min(bw / 1.5, bh / 1.5));
          text(guess[x].toUpperCase(), x * bw + bw / 2 + floor(y / 54) * width / 8, (y % 54) * bh + 52 + bh / 2);
        } 
      }
    ...
    for (let y = 0; y < 108; y++) {
      for (let x = 0; x < 5; x++) {
        ...
        if (y < game.guesses.length) {
          fill(0);
          textSize(min(bw / 1.5, bh / 1.5));
          text(game.guesses[y][x].toUpperCase(), x * bw + bw / 2 + floor(y / 54) * width / 8, (y % 54) * bh + 52 + bh / 2);
        }
      }
  ...
  for (let i = 0; i < 10; i++) {
    fill(230);
    rect(width / 2 + i * 45, height - 140, 40, 40);
    image(keyboardImages[letters[keyboardi]], width / 2 + i * 45, height - 140, 40, 40);
    fill(0);
    text(letters[keyboardi].toUpperCase(), width / 2 + i * 45 + 20, height - 140 + 20);
    keyboardi ++;
  }

  for (let i = 0; i < 9; i++) {
    fill(230);
    rect(width / 2 + i * 45 + 25, height - 95, 40, 40);
    image(keyboardImages[letters[keyboardi]], width / 2 + i * 45 + 25, height - 95, 40, 40);
    fill(0);
    text(letters[keyboardi].toUpperCase(), width / 2 + i * 45 + 45, height - 95 + 20);
    keyboardi ++;
  }

  for (let i = 0; i < 7; i++) {
    fill(230);
    rect(width / 2 + i * 45 + 50, height - 50, 40, 40);
    image(keyboardImages[letters[keyboardi]], width / 2 + i * 45 + 50, height - 50, 40, 40);
    fill(0);
    text(letters[keyboardi].toUpperCase(), width / 2 + i * 45 + 70, height - 50 + 20);
    keyboardi ++;
  }
  ...
  if (getCompletedWords() == 100) {
    ...
    fill(255);
    stroke(0);
    rect(width / 2 - 200, height / 2 - 300, 400, 360);

    fill(0);
    noStroke();
    textAlign(CENTER, CENTER);
    textSize(32);
    text("Congratulations!", width / 2, height / 2 - 260);

    textSize(24);
    text("You got it in:", width / 2, height / 2 - 200);

    textSize(64);
    text(game.guesses.length, width / 2, height / 2 - 140);

    fill(100, 100, 200);
    stroke(0);
    rect(width / 2 - 180, height / 2 - 40, 360, 80);

    fill(255);
    noStroke();
    textSize(32);
    text("COPY MY SCORE!", width / 2, height / 2);
  } else if (game.guesses.length >= 108) {
    fill(255, 200);
    rect(0, 0, width, height);
    
    fill(255);
    stroke(0);
    rect(width / 2 - 200, height / 2 - 300, 400, 140);

    fill(0);
    noStroke();
    textAlign(CENTER, CENTER);
    textSize(32);
    text("Aww, you lost!", width / 2, height / 2 - 260);

    text("Better luck next time!", width / 2, height / 2 - 200);
  }
